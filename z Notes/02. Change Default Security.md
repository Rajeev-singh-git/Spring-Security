...
4. [Disabling formLogin() and httpBasic()](#24-Disabling-formLogin()-and-httpBasic())
...
- **API path names are case-sensitive** — must exactly match in security configurations.
...
- Default `SecurityFilterChain` bean defines:
  - `authorizeHttpRequests().anyRequest().authenticated()` → secures all endpoints.
  - `formLogin(withDefaults())` → enables default login page.
  - `httpBasic(withDefaults())` → enables basic auth for API clients.
- Copied same logic into custom class `ProjectSecurityConfig` under `com.eazybytes.config`.
- Custom bean overrides framework’s default once defined.
...
💡 **Insight:**  
Defining a custom `SecurityFilterChain` bean gives complete control over API access rules and replaces Spring’s built-in defaults.

⚠️ **Gotcha:**  
If you replicate the framework’s defaults exactly, behavior remains unchanged — customization takes effect only after modifying the authorization chain.

---

### 2.3 Customizing Spring Security Rules

- `anyRequest()` + `permitAll()` or `denyAll()` overrides all rules → makes app fully public or blocked.
- `requestMatchers()` enables selective security control per endpoint:
  - `/myAccount`, `/myBalance`, `/myLoans`, `/myCards` → `authenticated()`
  - `/contact`, `/notices`, `/error` → `permitAll()`
...
💡 **Insight:**  
`requestMatchers()` enables fine-grained control over API access—balancing public and secured routes based on business needs.
...
- Disabling **formLogin()** → removes Spring’s default login page.

- Disabling **httpBasic()** → disables the browser popup / header-based authentication.

- Disabling both → no authentication entry point; all secured endpoints return **403 Forbidden**.
...
Each works through its own authentication filter — disabling one removes that login route.

⚠️ **Gotcha:**  
If both are disabled, Spring Security has no authentication path — all protected requests fail with 403.
